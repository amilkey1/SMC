...
namespace proj {
    ...
    class Likelihood {
        ...
        public:
            ...
            double                                  calcLogLikelihood(Forest::SharedPtr t);
        ...
        private:
            ...
            void                                    defineOperations(Forest::SharedPtr t);
            void                                    updateTransitionMatrices();
            void                                    calculatePartials();
            double                                  calcInstanceLogLikelihood(InstanceInfo & inst, Forest::SharedPtr t);
    ...
    inline void Likelihood::finalizeBeagleLib(bool use_exceptions) {  ///begin_finalizeBeagleLib
        ...
        for (auto info : _instances) {
            if (info.handle >= 0) {
                ...
                if (code != 0) {
                    if (use_exceptions)
                        throw XProj(boost::format("Likelihood failed to finalize BeagleLib instance. BeagleLib error code was %d (%s).") % code % _beagle_error[code]);
                    else
    ...
    inline void Likelihood::newInstance(unsigned nstates, int nrates, std::vector<unsigned> & subset_indices) {  ///begin_newInstance
        ...
        if (inst < 0) {
            ...
            throw XProj(boost::str(boost::format("Likelihood init function failed to create BeagleLib instance (BeagleLib error code was %d)") % _beagle_error[inst]));
        }
    ...
    inline void Likelihood::setTipStates() {   ///begin_setTipStates
        ...
        for (auto & info : _instances) {
            ...
            if (code != 0)
                throw XProj(boost::format("failed to set tip state for taxon %d (\"%s\"; BeagleLib error code was %d)") % (t+1) % _data->getTaxonNames()[t] % code % _beagle_error[code]);
            ++t;
            }
        }
    }   ///end_setTipStates
    ...
    inline void Likelihood::setAmongSiteRateHeterogenetity() {  ///begin_setAmongSiteRateHeterogenetity
        ...
        for (auto & info : _instances) {
            ...
            for (unsigned instance_specific_subset_index = 0; instance_specific_subset_index < info.subsets.size(); instance_specific_subset_index++) {
                ...
                if (code != 0)
                    throw XProj(boost::str(boost::format("Failed to set category rates for BeagleLib instance %d. BeagleLib error code was %d (%s)") % info.handle % code % _beagle_error[code]));
    ...
    inline void Likelihood::setModelRateMatrix() {  ///begin_setModelRateMatrix
        ...
        for (auto & info : _instances) {
            ...
            for (unsigned instance_specific_subset_index = 0; instance_specific_subset_index < info.subsets.size(); instance_specific_subset_index++) {
                ...
                if (code != 0)
                    throw XProj(boost::str(boost::format("Failed to set state frequencies for BeagleLib instance %d. BeagleLib error code was %d (%s)") % info.handle % code % _beagle_error[code]));
...
//        if (t->_is_rooted)
//            throw XProj("This version of the program can only compute likelihoods for unrooted trees");

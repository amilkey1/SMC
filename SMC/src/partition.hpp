...
namespace proj {
    ...
    inline void Partition::parseSubsetDefinition(std::string & s) {
        ...
        if (!matched) {
            throw XProj(boost::format("Could not interpret \"%s\" as a subset label with optional data type in square brackets") % before_colon);
        }
        ...
        if (match_obj.size() == 4 && match_obj[3].length() > 0) {
            ...
            if (std::regex_match(datatype, m, re)) {
                ...
                throw XProj(boost::format("Datatype \"%s\" specified for subset(s) \"%s\" is invalid: must be either nucleotide, codon, protein, or standard") % datatype % subset_name);
                }
            }
    ...
    inline void Partition::addSubsetRange(unsigned subset_index, std::string range_definition) {
        ...
        if (!matched) {
            throw XProj(boost::format("Could not interpret \"%s\" as a range of site indices") % range_definition);
        }
    ...
    inline int Partition::extractIntFromRegexMatch(regex_match_t s, unsigned min_value) {
        ...
        if (s.length() > 0) {
            ...
            if (int_value < (int)min_value) {
                throw XProj(boost::format("Value specified in partition subset definition (%d) is lower than minimum value (%d)") % int_value % min_value);
            }
        }
        return int_value;
    }
    ...
    inline void Partition::finalize(unsigned nsites) {
        ...
        if (_num_sites != nsites) {
            throw XProj(boost::format("Number of sites specified by the partition (%d) does not match actual number of sites (%d)") % _num_sites % nsites);
        }

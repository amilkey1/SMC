...
* `SMC` may scale better with larger numbers of loci than species due to larger numbers of particles needed for more species and memory limitations. For larger numbers of species, consider breaking the problem into smaller datasets.
...
`mcmc`: set to true to propose MCMC moves between filtering steps. Will increase number of likelihood calculations required but may improve variation.
